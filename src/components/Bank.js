...
//         const response = await axios.get(`https://pay.moonpayx.com/users/get/${token}`);
...
//     try {
//       const token = localStorage.getItem('token');
//       await axios.delete(`https://pay.moonpayx.com/users/del/${token}/accounts/${accountNumber}`);
...
useEffect(() => {
  const fetchAccounts = async () => {
    ...
    try {
      const response = await axios.get(`https://pay.moonpayx.com/account-details/bank/all/${email}`);
...
const handleDelete = async (id) => {
  ...
  try {
    const token = localStorage.getItem('token');
    await axios.delete(`https://pay.moonpayx.com/account-details/${countryObject[selectedCountry].urlName}/${id}`);
    toast.success('Account deleted successfully!');
    const response = await axios.get(`https://pay.moonpayx.com/account-details/bank/all/${token}`);
    const filteredData = response.data.filter(val => val.Country === countryObject[selectedCountry].name);
    setAccounts(filteredData.reverse());
  } catch (error) {
    console.error('Error deleting account:', error);
          ...
          accounts.length === 0  && !showForm  ? (
            ...
            <Formik
              ...
              onSubmit={async (values, { setSubmitting, resetForm }) => {
                ...
                const url = `https://pay.moonpayx.com/account-details/${countryObject[selectedCountry].urlName}/add`;
