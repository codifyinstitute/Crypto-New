...
//         const response = await axios.get(`http://147.93.20.176:8000/users/get/${token}`);
...
//     try {
//       const token = localStorage.getItem('token');
//       await axios.delete(`http://147.93.20.176:8000/users/del/${token}/accounts/${accountNumber}`);
...
useEffect(() => {
  const fetchAccounts = async () => {
    ...
    try {
      const response = await axios.get(`http://147.93.20.176:8000/account-details/bank/all/${email}`);
...
const handleDelete = async (id) => {
  ...
  try {
    const token = localStorage.getItem('token');
    await axios.delete(`http://147.93.20.176:8000/account-details/${countryObject[selectedCountry].urlName}/${id}`);
    toast.success('Account deleted successfully!');
    const response = await axios.get(`http://147.93.20.176:8000/account-details/bank/all/${token}`);
    const filteredData = response.data.filter(val => val.Country === countryObject[selectedCountry].name);
    setAccounts(filteredData.reverse());
  } catch (error) {
    console.error('Error deleting account:', error);
          ...
          accounts.length === 0  && !showForm  ? (
            ...
            <Formik
              ...
              onSubmit={async (values, { setSubmitting, resetForm }) => {
                ...
                const url = `http://147.93.20.176:8000/account-details/${countryObject[selectedCountry].urlName}/add`;
