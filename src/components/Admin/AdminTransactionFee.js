...
const AdminTransactionFee = () => {
    ...
    const fetchTransactionFee = async () => {
        ...
        // console.log(con)
        try {
            const response = await fetch(`http://147.93.20.176:8000/static/${countryObject[country]?.urlName}/one`);
            if (!response.ok) {
    ...
    const handleFeeUpdate = async (event) => {
        ...
        try {
            const response = await fetch(`http://147.93.20.176:8000/static/${countryObject[country]?.urlName}/put/${id}`, {
                method: 'PUT',
                headers: {
